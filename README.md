...
**Status: agent is feature-complete. First tagged release coming soon.**
...
new vLLM version), and `version`. No release has been tagged yet, so
`install.sh` has nothing to fetch. Build from source for now.
